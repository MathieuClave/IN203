...
void simulation(bool affiche)
{
    ...
    std::cout <<"[MPI process "<< rank << "] I am now MPI process "<< new_rank << " in subcommunicator " << subcommunicator << std::endl;

    if(subcommunicator == 'A'){ //Affichage
    ...
    if (subcommunicator == 'C'){ //Calcul
        ...
        while (!quitting){
            ...
            if (flag == 1) { //Si l'affichage attends
